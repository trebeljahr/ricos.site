...
const _characterLinvel = new Vector3();
const _characterTranslation = new Vector3();
...
export const Player = (props: RigidBodyProps) => {
  ...
  useBeforePhysicsStep(() => {
    ...
    if (characterRigidBody.translation().y < -50) {
      characterRigidBody.setTranslation(new Vector3(0, 25, 0), true);
      return;
    }
    ...
    const { forward, backward, leftward, rightward, jump, run } =
      getKeyboardControls() as SupportedControllerKeys;

    const speed = 0.1 * (run ? 1.5 : 1);
    ...
    const movement = characterController.current.computedMovement() as Vector3;
  ...
  useFrame((_, delta) => {
    ...
    _characterLinvel.copy(characterRigidBody.linvel() as Vector3);
  ...
  return (
    <>
      ...
      <PerspectiveCameraComponent makeDefault position={props.position} />
    </>
  );
};
