...
// Reused constants from FirstPersonController
...
export const FirstPersonControllerWithJoystick = ({
  enableJoystick = true,
  ...props
  ...
  // Store joystick movement data
  const joystickMovement = useRef<{ x: number; y: number }>({ x: 0, y: 0 });
  ...
  const handleCameraJoystick = (data: JoystickData) => {
    ...
    // Camera rotation is already handled by the JoystickControl component
    // for the first-person mode
  };
  ...
  useFrame((state, delta) => {
    ...
    // Use joystick input if available and enabled, otherwise use keyboard
    if (
      enableJoystick &&
      (Math.abs(joystickMovement.current.x) > 0.1 ||
        Math.abs(joystickMovement.current.y) > 0.1)
    ) {
      // Use joystick input
      frontVector.set(0, 0, -joystickMovement.current.y);
      sideVector.set(-joystickMovement.current.x, 0, 0);
    } else {
      // Use keyboard input
