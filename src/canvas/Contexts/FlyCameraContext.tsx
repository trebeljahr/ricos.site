...
const defaultFlyCameraState = {
  ...
  activate: function (position: vec3 | null = null, quaternion: quat2 | null = null) {
    this.active = true;
    ...
    if (position !== null && quaternion !== null) {
      ...
      vec3.transformQuat(rotatedForward, rotatedForward, quaternion as ReadonlyQuat);
      ...
      if (vec3.dot(rotatedForward, vec3.fromValues(1, 0, 0)) < 0) this.rotateY *= -1;

      // Rotation X
      this.rotateX = vec3.angle(rotatedForward, rotatedYForward);

      if (vec3.dot(rotatedForward, vec3.fromValues(0, 1, 0)) > 0) this.rotateX *= -1;
    }
  },
  deactivate: function () {
    this.active = false;
...
export const FlyCameraContextProvider = ({ children }: PropsWithChildren) => {
  ...
  return <FlyCameraContext.Provider value={value}>{children}</FlyCameraContext.Provider>;
};
...
export const useFlyCameraState = () => {
  ...
  useEffect(() => {
    ...
    const { gameUp, forward, rightward, upward, backward, leftward, downward } = fly;
